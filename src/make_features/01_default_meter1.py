...
class building_id_meter1(Feature):
    def create_features(self):
        self.train[self.__class__.__name__] = train['building_id']
        self.test[self.__class__.__name__] = test['building_id']


class meter_meter1(Feature):
    def create_features(self):
        self.train[self.__class__.__name__] = train['meter']
        self.test[self.__class__.__name__] = test['meter']


class timestamp_meter1(Feature):
    def create_features(self):
        train_date = pd.to_datetime(train['date'], format='%Y-%m-%d')
        test_date = pd.to_datetime(test['date'], format='%Y-%m-%d')

        self.train['timestamp_month_meter1'] = train_date.apply(lambda x: x.month)
        self.test['timestamp_month_meter1'] = test_date.apply(lambda x: x.month)

        self.train['timestamp_day_meter1'] = train_date.apply(lambda x: x.day)
        self.test['timestamp_day_meter1'] = test_date.apply(lambda x: x.day)

        self.train['timestamp_weekday_meter1'] = train_date.apply(lambda x: x.weekday())
        self.test['timestamp_weekday_meter1'] = test_date.apply(lambda x: x.weekday())
        ...
        self.train['timestamp_isholiday_meter1'] = np.where(train_date.isin(holidays), 1, 0)
        self.test['timestamp_isholiday_meter1'] = np.where(test_date.isin(holidays), 1, 0)


class site_id_meter1(Feature):
    def create_features(self):
        self.train[self.__class__.__name__] = train['site_id']
        self.test[self.__class__.__name__] = test['site_id']


class primary_use_meter1(Feature):
    def create_features(self):
...
class site_id_primary_use_meter1(Feature):
    def create_features(self):
        whole = pd.concat([train, test], axis=0)
...
class square_feet_div_mean_by_site_id_meter1(Feature):
    def create_features(self):
        whole = pd.concat([train, test], axis=0)
...
class square_feet_div_mean_by_primary_use_meter1(Feature):
    def create_features(self):
        whole = pd.concat([train, test], axis=0)
...
class square_feet_div_mean_by_year_built_meter1(Feature):
    def create_features(self):
        whole = pd.concat([train, test], axis=0)
