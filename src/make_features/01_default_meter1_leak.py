...
class building_id_meter1_leak(Feature):
    def create_features(self):
        self.train[self.__class__.__name__] = train['building_id']
        self.test[self.__class__.__name__] = test['building_id']


class meter_meter1_leak(Feature):
    def create_features(self):
        self.train[self.__class__.__name__] = train['meter']
        self.test[self.__class__.__name__] = test['meter']


class timestamp_meter1_leak(Feature):
    def create_features(self):
        train_date = pd.to_datetime(train['date'])
        test_date = pd.to_datetime(test['date'])

        self.train['timestamp_month_meter1_leak'] = train_date.apply(lambda x: x.month)
        self.test['timestamp_month_meter1_leak'] = test_date.apply(lambda x: x.month)

        self.train['timestamp_day_meter1_leak'] = train_date.apply(lambda x: x.day)
        self.test['timestamp_day_meter1_leak'] = test_date.apply(lambda x: x.day)

        self.train['timestamp_weekday_meter1_leak'] = train_date.apply(lambda x: x.weekday())
        self.test['timestamp_weekday_meter1_leak'] = test_date.apply(lambda x: x.weekday())

        self.train['timestamp_hour_meter1_leak'] = train_date.apply(lambda x: x.hour)
        self.test['timestamp_hour_meter1_leak'] = test_date.apply(lambda x: x.hour)


class site_id_meter1_leak(Feature):
    def create_features(self):
        self.train[self.__class__.__name__] = train['site_id']
        self.test[self.__class__.__name__] = test['site_id']


class primary_use_meter1_leak(Feature):
    def create_features(self):
...
class site_id_primary_use_meter1_leak(Feature):
    def create_features(self):
        whole = pd.concat([train, test], axis=0)
...
class square_feet_div_mean_by_site_id_meter1_leak(Feature):
    def create_features(self):
        whole = pd.concat([train, test], axis=0)
...
class square_feet_div_mean_by_primary_use_meter1_leak(Feature):
    def create_features(self):
        whole = pd.concat([train, test], axis=0)
...
class square_feet_div_mean_by_year_built_meter1_leak(Feature):
    def create_features(self):
        whole = pd.concat([train, test], axis=0)
